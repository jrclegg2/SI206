...
# define uprint to handle errors from tweepy
import sys
...
def get_tweets():
    if "umsi" in CACHE_DICTION:
        ...
        try:
            CACHE_DICTION['umsi'] = api.search(q = 'umsi')
...
cur = conn.cursor()
# conn.commit() to commit all changes, then conn.close() to finish
...
for tweet in umsiTweetsDict['statuses']:
    tweet_id = tweet['id']
    author = tweet['user']['screen_name']
    time_posted = tweet['created_at']
    tweet_text = tweet['text']
    retweets = tweet['retweet_count']
    cur.execute("INSERT INTO Tweets VALUES (?, ?, ?, ?, ?)", (tweet_id, author, time_posted, tweet_text, retweets))
