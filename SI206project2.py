...
# I HAVE DEFINED TWO HELPER FUNCTIONS TO GET THIS DATA:
def get_umsi_names(url):
    listNames = []
    html = requests.get(url, headers = {"User-Agent" :"SI_CLASS"}).text
    soup = BeautifulSoup(html, 'html.parser')
    nameTags = soup ("div", class_ = "field field-name-title field-type-ds field-label-hidden") # property = "dc:title", class_ = "field-item even")
    for tag in nameTags:
        name = tag.text
        listNames.append(name)
    return listNames
# defining function to grab titles
def get_umsi_titles(url):
    listTitles = []
    html = requests.get(url, headers = {"User-Agent" :"SI_CLASS"}).text
    soup = BeautifulSoup(html, 'html.parser')
...
def get_umsi_data():
    ...
    while (i < 13):
        ...
        url += "&page=" + str(i)
        allTitlesList += get_umsi_titles(url)
        allNamesList += get_umsi_names(url)
        i += 1
    dictionaryUmsiData = dict(zip(allNamesList,allTitlesList))
    return dictionaryUmsiData
...
def num_students(data):
    return sum(x == 'PhD student' for x in data.values())
